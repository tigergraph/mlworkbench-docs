...
TigerGraph Machine Learning (ML) Workbench is a Jupyter-based Python development framework offered either as a fully-managed, on-cloud offering or as an integrated on-premises solution for direct integration with existing machine learning infrastructure.
...
TigerGraph ML is designed to be infrastructure- and ML framework-agnostic, working on-prem or in a private cloud.

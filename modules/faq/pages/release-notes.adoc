...
== TigerGraph ML Workbench 1.4 Legacy
June 17, 2025

[IMPORTANT]
THe TigerGraph ML Workbench is being retired and will be replaced with an improved GNN training intergration.
...
(See xref:pytigergraph:release-notes:index.adoc[pyTigerGraph Release Notes])
...
* Two new xref:pytigergraph:gds:dataloaders.adoc[data loader classes]: `HGTLoader` and `NodePiece`.
* Callback functions to all xref:pytigergraph:gds:factory-functions.adoc[data loader factories]: users can write functions to process the batch in a background thread before it is passed into the training loop.
* A delimiter parameter to all xref:pytigergraph:gds:factory-functions.adoc[data loader factories]: users can choose what delimiter they want to separate attributes they are loading from the graph.

=== Updated features

* Template query support in the xref:pytigergraph:gds:featurizer.adoc[featurizer] (requires TigerGraph Database version 3.9+).

...
* If you are running aversion of JupyterLab older than 3.0, such as the JupyterLab version on AWS SageMaker, you need to <<_install_gdps, install GDPS manually>> on your TigerGraph server and <<_install_kernel_for_jupyterlab, install the `tigergraph-torch` kernel>> on your JupyterLab instance.
...
[#_install_kernel_for_jupyterlab]

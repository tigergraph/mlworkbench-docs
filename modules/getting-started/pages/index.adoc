...
* If you already have a running TigerGraph instance, you can xref:.installation.adoc[install ML Workbench] on your machine as a standalone application.
* If you already have your own TigerGraph instance and a JupyterLab server, either self-hosted or on a cloud machine learning environment, you can download our Python library and Jupyter extension to integrate the Workbench with your JupyterLab.
...
* If you are not using JupyterLab at all, but still want to perform machine learning tasks with the graph data in your TigerGraph database, you can use pyTigerGraph.


== Connect to TigerGraph Server through GDPS
...
* If you installed the ML Workbench and are launching JupyterLab from the `mlworkbench` environment, you can xref:deploy-gdps.adoc[deploy GDPS through the ML Workbench server manager] in the JupyterLab web interface.
* If you are using JupyterLab and your JupyterLab version is 3.0 or later, you can xref:jupyterlab-server/self-hosted.adoc[install the Workbench's JupyterLab extension as well as the `tigergraph-torch` Python kernel], and then xref:deploy-gdps.adoc[deploy GDPS] through the ML Workbench server manager.
